...
export async function startDiscordBot({
  ...
  const setupHandlers = async (c: Client<true>) => {
    ...
    if (projectDirectories.size > 0) {
      discordLogger.log(
        `[CACHE] Warming session cache for ${projectDirectories.size} project(s)...`,
      )
      void Promise.all(
        Array.from(projectDirectories).map((dir) => {
          return refreshSessionCache(dir).catch(() => {})
        }),
      ).then(() => {
        discordLogger.log(
          `[CACHE] Session cache warmed for ${projectDirectories.size} project(s)`,
        )
      })
    }
  }
  ...
  discordClient.on(Events.MessageCreate, async (message: Message) => {
    try {
      ...
      if (message.guild && message.member) {
        ...
        const hasNoDisundayRole = message.member.roles.cache.some(
        ...
        const hasDisundayRole = message.member.roles.cache.some(
      ...
      if (channel.type === ChannelType.GuildText) {
        ...
        const shouldUseWorktrees =
          useWorktrees || getChannelWorktreesEnabled(textChannel.id)
        ...
        if (shouldUseWorktrees) {
          ...
          if (getClient instanceof Error) {
            discordLogger.error(
              `[WORKTREE] Failed to init OpenCode: ${getClient.message}`,
            )
            setWorktreeError({
              threadId: thread.id,
            ...
            if (!clientV2) {
              discordLogger.error(
                `[WORKTREE] No v2 client for ${projectDirectory}`,
              )
              setWorktreeError({
                threadId: thread.id,
