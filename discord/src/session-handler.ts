...
export async function handleOpencodeSession({
  ...
  if (threadPermissions && threadPermissions.size > 0) {
    ...
    for (const [permId, pendingPerm] of threadPermissions) {
      ...
      if (!clientV2) {
        sessionLogger.log(
          `[PERMISSION] OpenCode v2 client unavailable for permission ${permId}`,
        )
  ...
  const promptResult:
    ...
    const modelParam = await (async () => {
      ...
      if (sessionModelPreference) {
        ...
        if (providerID && modelID) {
          sessionLogger.log(
            `[MODEL] Using session preference: ${sessionModelPreference}`,
          )
          return { providerID, modelID }
        }
      }
      ...
      if (agentPreference) {
        ...
        if (!(agentsResponse instanceof Error) && agentsResponse.data) {
          ...
          sessionLogger.log(
            `[MODEL] Agent "${agentPreference}" has no model configured`,
          )
        }
      }
      ...
      if (channelModelPreference) {
        ...
        if (providerID && modelID) {
          sessionLogger.log(
            `[MODEL] Using channel preference: ${channelModelPreference}`,
          )
          return { providerID, modelID }
        }
      }
      ...
      if (defaultModel) {
        sessionLogger.log(
          `[MODEL] Using default: ${defaultModel.providerID}/${defaultModel.modelID}`,
        )
        return defaultModel
      }
      ...
      sessionLogger.log(
        `[MODEL] No model available (no preference, no default)`,
      )
      return undefined
    })()
