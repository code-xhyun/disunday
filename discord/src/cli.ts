...
async function checkSingleInstance(): Promise<void> {
  ...
  try {
    ...
    if (response.ok) {
      cliLogger.log(
        `Another disunday instance detected for data dir: ${getDataDir()}`,
      )
...
async function registerCommands({
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName('add-project')
      .setDescription(
        'Create Discord channels for a project. Use `npx disunday add-project` for unlisted projects',
      )
      .addStringOption((option) => {
        option
          .setName('project')
          .setDescription(
            'Select a project. Use `npx disunday add-project` if not listed',
          )
          .setRequired(true)
          .setAutocomplete(true)
...
function showReadyMessage({
  ...
  if (allChannels.length > 0) {
    ...
    note(
      `Your disunday channels are ready! Click any link below to open in Discord:\n\n${channelLinks}\n\nSend a message in any channel to start using OpenCode!`,
      '🚀 Ready to Use',
    )
  }

  note(
    'Leave this process running to keep the bot active.\n\nIf you close this process or restart your machine, run `npx disunday` again to start the bot.',
    '⚠️  Keep Running',
  )
}
              ...
              .then(async (roles) => {
                ...
                if (existingRole) {
                  ...
                  if (existingRole.position > 1) {
                    ...
                    cliLogger.info(
                      `Moved "Disunday" role to bottom in ${guild.name}`,
                    )
                  }
                  return
                }
...
cli
  ...
  .option(
    '--data-dir <path>',
    'Data directory for config and database (default: ~/.disunday)',
  )
    ...
    async (options: {
      ...
      try {
        ...
        if (options.installUrl) {
          ...
          if (!existingBot) {
            cliLogger.error(
              'No bot configured yet. Run `disunday` first to set up.',
            )
            process.exit(EXIT_NO_RESTART)
          }
...
cli
  ...
  .action(async (files: string[], options: { session?: string }) => {
    try {
      ...
      if (!botRow) {
        cliLogger.error(
          'No bot credentials found. Run `disunday` first to set up the bot.',
        )
        process.exit(EXIT_NO_RESTART)
      }
  ...
  .action(
    async (options: {
      ...
      try {
        ...
        if (channelId) {
          const channelArgIndex = process.argv.findIndex(
            (arg) => arg === '--channel' || arg === '-c',
          )
          ...
          try {
            ...
            if (existingChannel) {
              ...
              if (!appId) {
                ...
                cliLogger.error(
                  'App ID is required to create channels. Use --app-id or run `disunday` first.',
                )
                process.exit(EXIT_NO_RESTART)
              }
        ...
        if (prompt.length > DISCORD_MAX_LENGTH) {
          ...
          try {
            ...
            starterMessage = (await starterMessageResponse.json()) as {
              id: string
            }
          } finally {
            // Clean up temp file
...
cli
  .command(
    ...
    'Create Discord channels for a project directory (e.g. ./folder)',
  )
  .option(
    '-g, --guild <guildId>',
    ...
    async (
      ...
      try {
        ...
        if (options.guild) {
          ...
          const guildArgIndex = process.argv.findIndex(
            (arg) => arg === '-g' || arg === '--guild',
          )
          const rawGuildArg =
