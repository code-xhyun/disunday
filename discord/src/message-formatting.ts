...
import { getFileValidationConfig } from './config.js'
  ...
  const fileAttachments = Array.from(message.attachments.values()).filter(
    (attachment) => {
      ...
      if (attachment.size > config.maxFileSizeBytes) {
        logger.warn(
          `Attachment ${attachment.name} exceeds size limit: ${attachment.size} > ${config.maxFileSizeBytes}`,
        )
        return false
      }
      ...
      if (!isAllowed && contentType) {
        logger.warn(
          `Attachment ${attachment.name} has disallowed MIME type: ${contentType}`,
        )
      }
